...
pub use crate::api_client::{ApiResult, BeaconApiLightClient, ApiResponseData, ApiClientError};
...
impl BeaconApiServer {
    pub fn from_config(config: &Settings) -> Self {
        ...
        let client: BeaconApiLightClient = BeaconApiLightClient::new(&config.node.url);
...
async fn serve_latest_header() -> ApiResponseData<String> {
    ...
    // println!("{:?}", response);
    ApiResponseData {data: String::from("test") }
}
